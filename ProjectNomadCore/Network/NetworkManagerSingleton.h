...
#include "NetworkManagerCallbackTypes.h"
#include "Secrets/NetworkSecrets.h"
#include "Utilities/LoggerSingleton.h"
#include "Utilities/Singleton.h"

namespace ProjectNomad {
    enum class NetworkManagerStatus : uint8_t { NotInitialized, Initialized, LoggedIn };
    ...
    class NetworkManagerSingleton {
        ...
        NetStatus currentNetStatus = NetStatus::NotInitialized;
...
#pragma region "Lifecycle"
        ...
        // Initialize based on EOS SDK sample app's FMain::InitPlatform()
        void initialize() {
            if (isInitialized()) {
            ...
            EOS_EResult setLogCallbackResult = EOS_Logging_SetCallback([](const EOS_LogMessage* message) {
                Singleton<NetworkManagerSingleton>::get().handleEosLogMessage(message);
            });
            ...
            if (tryCreatePlatformInstance()) {
                setNetStatus(NetStatus::Initialized);
            }
        }
        ...
        void cleanupState(bool forceShutdown) {
            ...
            if (forceShutdown) {
                if (isInitialized()) {
                    ...
                    currentNetStatus = NetStatus::NotInitialized;
        ...
        // Login with SDK DevAuth tools based on SDK sample's FAuthentication::Login
        void login(const std::string& devAuthName) {
            if (!isInitialized()) {
            ...
            EOS_Auth_Login(authHandle, &loginOptions, this, [](const EOS_Auth_LoginCallbackInfo* data) {
                Singleton<NetworkManagerSingleton>::get().loginCompleteCallback(data);
            });
        ...
        void logout() {
            ...
            EOS_Auth_Logout(authHandle, &LogoutOptions, nullptr, [](const EOS_Auth_LogoutCallbackInfo* data) {
                Singleton<NetworkManagerSingleton>::get().logoutCompleteCallback(data);
            });
        }
...
#pragma endregion
    ...
    private:
        ...
        void onSuccessfulCrossPlatformLogin(const EOS_ProductUserId resultCrossPlatformId) {
            ...
            setNetStatus(NetStatus::LoggedIn);
        ...
        void handleReceivedMessages() {
            ...
            EOS_EResult result = EOS_P2P_ReceivePacket(p2pHandle, &options, &peerId, &socketId, &channel, messageData.data(), &bytesWritten);
            ...
            {
                // Convert message to simple string
        ...
        void startCrossPlatformLogin() {
            ...
            if (result == EOS_EResult::EOS_Success) {
                ...
                EOS_Connect_Login(connectHandle, &options, nullptr, [](const EOS_Connect_LoginCallbackInfo* data) {
                    Singleton<NetworkManagerSingleton>::get().crossPlatformLoginCompleteCallback(data);
                });
        ...
        void subscribeToConnectionRequests() {
            ...
            connectionNotificationId = EOS_P2P_AddNotifyPeerConnectionRequest(p2pHandle, &options, nullptr,
                [](const EOS_P2P_OnIncomingConnectionRequestInfo* data) {
                    Singleton<NetworkManagerSingleton>::get().onIncomingConnectionRequest(data);
                });
...
#pragma region Status Helpers
        ...
        void setNetStatus(NetStatus newStatus) {
        ...
        bool isInitialized() {
            return currentNetStatus != NetStatus::NotInitialized;
        }
